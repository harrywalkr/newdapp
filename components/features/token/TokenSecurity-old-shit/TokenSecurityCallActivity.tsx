...
const Record = ({ data, idx }: { data: SmartContractCall; idx: number }) => {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <div className="flex justify-center items-center">
          <div className="border p-2 rounded-md text-base-100 min-w-[100px] text-center">
...
export default function TokenSecurityCallActivity({ tokenAddress }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://onchain.dextrading.com/functionTypeCalls?limit=100&network=${selectedChain.symbol.toLowerCase()}&address=${tokenAddress}&from=2018-01-14&till=${to.toISOString()}`,
    )
